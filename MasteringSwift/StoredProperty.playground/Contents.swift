...
struct BlogPost2 {
    ...
    /*
     ...
     var formattedDate: String 의 경우 date 속성에 접근하는 부분에서 오류 발생
     lazy 속성이 없으면 두 속성이 초기화되는 시점이 같아지는데, date 속성이 위에 있으니까 먼저 초기화 되고 아래쪽에서 사용 불가하다.
     문법적으로 허용되지 않는다.
     저장 속성을 클로저로 초기화 할 때 다른 속성에 접근해야한다면 반드시 지연 저장 속성 lazy 로 선언해야한다.
     */
}
